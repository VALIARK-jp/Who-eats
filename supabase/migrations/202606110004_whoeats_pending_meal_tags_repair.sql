...
-- Repair for prod where `list_pending_meal_tags` is still missing.
-- Keep this narrow, but include the companion table + column so the RPC has
-- its full dependency chain if prod missed an older migration.
