...
-- jsonb 返却は PostgREST 経由でパースが不安定なことがあるため、行セットで返す。
-- prefecture_code 列が未設定でも city_code 先頭2桁で都道府県を判定する。

drop function if exists public.whoeats_map_choropleth_prefecture(text);
...
as $$
  ...
  pref as (
    select lpad(trim(p_prefecture_code), 2, '0') as code
  ),
  visible_posts as (
    select
      p.user_id,
    ...
    where p.deleted_at is null
      ...
      and left(trim(pl.city_code), 2) = pr.code
      and (
        p.user_id = v.uid
